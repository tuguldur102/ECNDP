...
from algorithms.exact_path_formulation import solve_ecndp_path_pulp
  ...
  for terminal, k_budgets in tqdm(
    ...
    for k_budget in tqdm(
      ...
      S_exact, best_pc_exact, total_time_exact = solve_exact(
        G_assigned, K, terminals_assigned, case=case_number, algorithm=solve_ecndp_path_pulp, 
      )
      ...
      df.to_csv(f"{SAVE_PATH_ROOT}/Result_ECNDP_mammals_all_path.csv", index=False)
